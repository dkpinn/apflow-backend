-- Single-user organisation exemption for PDF/VLM bank statement approval.
--
-- The independent-reviewer requirement (gold fixture verifier ≠ upload approver)
-- is a segregation-of-duties control that is impossible to satisfy for a
-- one-person organisation. This migration relaxes that specific check when
-- the organisation has only one member, while keeping all quality gates:
--   - Corrected gold fixture must still exist
--   - approved_by must still be recorded (checklist approval required)
--   - Benchmark pass is still required
--   - All balance/duplicate checks are unchanged
--
-- Multi-user organisations are unaffected.
...
BEGIN
  ...
  THEN
    -- Gold fixture required unconditionally.
    SELECT count(*)
    ...
    -- Manual approval required unconditionally.
    ...
    -- Independent verifier check — waived for single-user organisations.
    SELECT count(*)
      INTO org_member_count
