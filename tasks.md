# Tasks
- Invoices in accordians must only appear in one list.
...
- on the supplier master/accounts number, tax number, move the supplier code to a bottom accordian.
- add supplier is no longer working from the detailed invoice section.
